#!/usr/bin/env python3
"""
Quick evaluation of the balanced BIT model with sampled datasets.

This script samples from the large datasets to quickly verify:
- deepset benign FPR dropped from 40.2% to <5%
- Overall FPR dropped from 37.1% to <5%
"""
...
def main():
    ...
    print("=== Quick Balanced BIT Model Evaluation ===")
    ...
    # Save results
    results_file = Path("results/quick_balanced_evaluation.json")
    ...
    if deepset_benign_results["fpr"] < 0.05:
        ...
        print(f"❌ deepset benign FPR still high: {deepset_benign_results['fpr']*100:.1f}%")

    print(f"✅ Balanced training with 67% safe benign samples")
    print(f"✅ HTML preprocessing implemented (not tested in this quick eval)")
