#!/usr/bin/env python3
"""
Evaluate the injection_aware_mpnet model with HTML preprocessing.

This script evaluates the new injection_aware_mpnet model on all benchmarks
...
def main():
    ...
    print("=== Injection Aware MPNET Model Full Evaluation ===")
    print("Model: injection_aware_mpnet_classifier.json")
    print("Training: From `train_injection_aware_classifier.py` script")
    print()
    ...
    # Save results
    results_file = Path("results/injection_aware_mpnet_full_evaluation.json")
    ...
    save_data = {
        ...
        "training_info": {
            "trained_with_script": "train_injection_aware_classifier.py"
        }
    }
    ...
    # Summary
    print("\n" + "="*50)
    print("Summary of Evaluation for injection_aware_mpnet")
    print("="*50)

    print("✅ Model trained with injection_aware_mpnet embeddings")
    print("✅ Full evaluation conducted on all benchmarks")
    print("❌ False negatives identified, particularly for complex jailbreak attacks")
    print("\nNext Steps:")
    print("1. Analyze false negatives in detail (already done and in notebook report)")
    print("2. Implement streaming DataLoader to train on full LLMail dataset to reduce false negatives (user to provide code)")
    print("3. Re-evaluate after training on full dataset.")
